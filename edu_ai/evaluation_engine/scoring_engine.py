...
def format_json_output(sem_score, concept_score, kw_score, final_score, marks, max_marks, 
    ...
    """Formats the final evaluation into structured JSON with backward-compatible keys."""
    result = {
        # ── BACKWARD-COMPATIBLE TOP-LEVEL KEYS (used by app.py) ──
        ...
        "analysis": {
            ...
            "missing_concepts": missing_concepts if missing_concepts else [],
            "contradictions_detected": contradictions if contradictions else [],
            "factual_errors": [m for m in (missing_concepts or []) if "Error" in m or "Contradiction" in m],
