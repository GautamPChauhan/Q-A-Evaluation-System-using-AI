...
def student_exams():
    ...
    return render_template('student_exams.html', questions=questions, name=session.get('email'))
...
def student_results():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT e.evaluation_id, q.question_text, a.answer_text, e.score, e.feedback, e.evaluated_at "
            "FROM evaluations e "
    ...
    return render_template('student_results.html', results=results, name=session.get('email'))
